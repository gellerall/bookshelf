...
def create_all():
    with sqlite3.connect('bookshelf.db') as db:
        try:
            cursor = db.cursor()
            cursor.execute("""
            ...
            query_author = 'INSERT INTO author (author_id, name, description) values(?, ?, ?)'
            data_author = [
            ...
            cursor.executemany(query_author, data_author)

            query_status = 'INSERT INTO status (status_id, date_start, date_end, name_status, is_actual) values(?, ?, ?, ?, ?)'
            data_status = [
            ...
            cursor.executemany(query_status, data_status)

            query_book = 'INSERT INTO book (book_id, name_book, comments, grade, name_author, actual_status) values(?, ?, ?, ?, ?, ?)'
            data_book = [
            ...
            cursor.executemany(query_book, data_book)

            db.commit()
        except sqlite3.Error as e:
            if db: db.rollback()
            print("Ошибка выполнения запроса на создание таблиц. Вероятно, таблица уже существует.")


def print_dairy_data():
     with sqlite3.connect('bookshelf.db') as db:
        try:
            cursor = db.cursor()
            query_data_book = '''
                                ...
                                , grade
                                , name_status 
                                from book b inner join status s on b.actual_status = s.status_id 
                                inner join author a on a.author_id = b.name_author
                                where name_status = "Прочитана"
                                '''
            data = cursor.execute(query_data_book)
            data_arr = []
            for book in data:
                data_arr.append(book)
            return data_arr
        except sqlite3.Error as e:
            if db: db.rollback()
